...
where $\Phi_0=2\pi\hbar e/c=he/c$ is the flux quantum (see {ref}`sec-magfield-6`). Thus if the magnetic flux in the solenoid is an integer multiple of a flux quantum then all these phase factors become unity, and the propagator becomes
...
Consider a spinless electron moving in the uniform magnetic field $\Bvec=B\zvechat$.  Let $n$ be the quantum number of $H_\perp$ (the Landau level), and let $P_3$ be the eigenvalue of operator ${\hat P}_3$, as in Eq.~(\eqr\cn.13).
...
**(a)**  Express $L$ in terms of variables $(X,Y,v_x,v_y)$ and then in terms of $(Q_1,P_1,Q_2,P_2)$. Show that it commutes with both $H_\perp$ and ${\hat P}_3$, so that all three operators, $(L,H_\perp,{\hat P}_3)$ commute with one another. Let the eigenvalues of $L$ be written $m\hbar$. Show that $m$ is an integer. For given $n$, what are the allowed values of $m$?
...
**(c)**  Now write out differential operators in the $w$, $\bar w$ coordinates for the creation operators $a^\dagger$ and $b^\dagger$. Use them to obtain the normalized wave function $\phi_{kn}$ for $k=0$, $n>0$.

**(d)**  Use the Rodriguez formula for the associated Laguerre polynomials,
...
Consider the motion of an electron of charge $q=-e$ in the field of a magnetic monopole.  Assume the monopole is infinitely massive in comparison to the electron.  The monopole produces a field,
...
**(a)**  Write down Newton's laws for the electron motion. You may use the abbreviation,
...
**(b)**  Consider the vector potential,
...
**(c)**  By playing around with dot products of various vectors and looking for exact time derivatives, find more constants of motion. Use these to find $r^2$ as a function of $t$. Show that the electron can reach the singularity at $r=0$ only if $L^2=0$.

**(d)**  Show that the orbit lies on a cone whose apex is at the origin. Find the opening angle of the cone as a function of $(E,\Jvec)$. Find a vector which specifies the axis of the cone. Assume $L^2\ne0$, and let $t=0$ occur at the point of closest approach. Find the distance of closest approach as a function of $(E,L)$.

**(e)**  Choose the axes so that $\Jvec$ is parallel to the $z$-axis. Find $(r,\theta,\phi)$ as functions of $t$.
...
This problem is relevant for the quantum Hall effect.  Consider the motion of a spinless particle of charge $q=-e$ in the magnetic field $\Bvec=B\zvechat$ and electric field $\Evec=-E\xvechat$, where $B$ and $E$ are positive constants (the fields are uniform).  In this problem we ignore the motion in the $z$-direction, and just work with a 2-dimensional problem in the $x$-$y$ plane.
...
**(a)**  Express the Hamiltonian as a function of the operators $({\hat X},{\hat Y},{\hat v}_x,{\hat v}_y)$. In the case of the uniform magnetic field (without $\Evec$) we found that $H$ commuted with any function of $\hat X$ or $\hat Y$. What functions of $\hat X$ and $\hat Y$ does $H$ commute with now that we have an electric field? What is the energy of a state that is an eigenstate of both $\hat X$ and $H$? Express the energy in terms of some reasonable quantum number(s).

**(b)**  Choose a gauge $\Avec = Bx{\hat{\mathbf{y}}}$, where ${\hat{\mathbf{y}}}$ is the unit vector in the $y$-direction. Let ${\hat X}\psi(x,y) = X\psi(x,y)$. Find the most general wave function $\psi(x,y)$ that satisfies this equation.

**(c)**  Now let $H\psi(x,y)=K\psi(x,y)$, where $K$ is the energy, and where $\psi$ has the form determined in part (b). Express the solution $\psi(x,y)$ in terms of $u_n(x)$, defined to be the normalized harmonic oscillator eigenfunction for quantum number $n$.

**(d)**  Compute the $y$-component $J_y$ of the probability flux, properly interpreted as a particle flux since the wave function is not normalizable. Integrate this flux over the line $y=const$ to get the number of particles/unit time crossing this line. Notice the relation to the drift velocity.

(prob-magfield-4)=

**Problem 4.** 

This problem has some things in common with Prob.~{ref}`prob-magfield-1`, but the notation is independent of that of that problem.

**(a)**  Consider an electron moving in the magnetic field $\Bvec = B{\hat{\mathbf{z}}}$. We ignore the spin of the electron (we treat it as if it were a spinless particle), and we ignore the motion in the $z$-direction. Thus, we have a 2-dimensional problem, in the $x$-$y$ plane.
...
**(c)**  If the initial conditions for the classical motion are $v_x(0)=0$, $v_y(0)=v_0$, find the classical solutions $v_x(t)$, $v_y(t)$.

**(d)**  Let $\psi(x,y,t)$ be a solution of the time-dependent Schr\"odinger equation. We are interested in the solution for which the initial condition $\psi(x,y,0)$ is the wave function corresponding to the state
